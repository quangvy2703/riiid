...
class Trainer:
    ...
    def _train(self, batch_iter, num_batches):
        ...
        outs = []
        try:
            for batch in tqdm(batch_iter, total=num_batches, desc="Training..."):
            ...
            print(batch['label'].min(), batch['label'].max(), batch['input'].min(), batch['input'].max(), batch['target_id'].min(), batch['target_id'].max())
